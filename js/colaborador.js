...
const mesAno = new Intl.DateTimeFormat('pt-BR', { month:'2-digit', year: 'numeric' }).format(dataPost).replace('/','-')
const departamento = JSON.parse(localStorage.getItem('colaborador'));
const iosFix = new URL((window.location.href)).searchParams.get('value')
const areaPrincipal = departamento.area == '' ? iosFix.split('_')[1] : departamento.area
...
fetch(`https://docs.google.com/spreadsheets/d/1MIgBa5iCrcZJUE6MX0g8L0BZfSTP_TfqYIh17DOP5HA/gviz/tq?&sheet=${desacentuar(areaPrincipal)}&range=A2:Z1000`)
...
.then(json => {
        ...
        planilha.map((dado) =>{
            ...
            const nome = primeiraLetra(nomeSobrenome(checarExistencia(0))),
            ...
            //'https://drive.google.com/file/d/1G86XfkwvjEqt8RdRel711pGZmTREtS4T/view?usp=sharing'
        ...
        const tempoKabum = (data) => {
            ...
            proximoAniversario = new Date(dataEntrada.getFullYear() + anosCasa + 1, dataEntrada.getMonth(), dataEntrada.getDate())
            ...
            if(anosCasa < 1 ){
                return {
                    ...
                    aniversario: proximoAniversario.getDate() == hoje.getDate() ? 'aniversariante' : ''
                }
            }else {
                return {
        ...
        if (departamento.nome == nome) {
            colaborador = 
                    `<div class="figure">
                        <img src="${foto}" class="foto" draggable="false" onerror="this.onerror=null;this.src='./img/${defaultNinja()}.webp'"/>
                    </div>
                    <div>
                        <div flex-top>
                            <div data-name>
                                <h2>${nome} <span>(@${email.split('@')[0]})</span></h2>
                                <a href="https://mail.google.com/mail/?view=cm&fs=1&to=${email}" target="_blank">${gMailIcon} E-mail</a>
                            </div>
                            <span data-info>${primeiraLetra(cargo)} | <b>${primeiraLetra(area)}</b></span>
                            <p data-bio>${bio}</p>
                        </div>
                        ...
                        <div data-badges>
                            ...
                            <span data-plus style="display:${condicaoBadge(13)}" onclick="handleAuthClick()">${plusIcon}  <span>Ninja</span> </span>
                        </div>
                    </div>`
                    //document.querySelector('.post').innerHTML = post
        } 
            document.querySelector('.colaborador').innerHTML = colaborador
            
    })
})


// const CLIENT_ID = '40031144465-qtjs68j4bd907g1ubqe93jitkv07202b.apps.googleusercontent.com';
// const API_KEY = 'AIzaSyBuAJkHWgJCJ9LZOL0uFqyAxihoL_gk9ok';
...
//     resp.error ? (resp) : await printDocTitle();
//   }
...
//       documentId: '1es3SbkQ5ziXcZPmnIhGbRvXaPOPCt7w_J-OdpCbe790',
//     });
//     const doc = response.result;
//     const output = `Document ${doc.title} successfully found.\n`;
    
//         let length = []
//         length.push(doc.body.content.length)
//         for(let item = 1; item < length; item++){
//             const conteudo = doc.body.content[item].paragraph.elements[0].textRun
//             console.log(conteudo)
//             document.querySelector('.post').innerHTML += `${conteudo.content}<br>`;
//         }

// }


// fetch(`./js/noposts.txt`).then(result => result.text()).then(text=>{

//         fetch(`./js/${mesAno}-${area.nome}-post.txt`).then(resultado =>
//             !resultado.ok ?
...
//             ).then(texto =>{
//                 document.querySelector('.post').innerHTML = texto;
